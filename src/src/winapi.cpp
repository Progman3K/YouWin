...
} EnumChildWindowsParams;


static BOOLEAN EnumChildren( Window * pWnd, LPARAM lParam ) {
    ...
    DBG_MSG( DBG_GENERAL_INFORMATION,
        ...
        pWnd->pClass->GetClassName(),
        pWnd,
        (unsigned long)( pWnd->hMenu ),
        pWnd->cx, pWnd->cy
    );
#endif
    EnumChildWindowsParams * pParams = (EnumChildWindowsParams *)lParam;

    if ( ( HWND_DESKTOP == pWnd->pParentWnd ) && ( NULL == pParams->pParentWnd ) ) {

        DBG_MSG( DBG_GENERAL_INFORMATION,
            TEXT(
            "Toplevel window found: "
            "%8s"
            " %8X"
            " %5d"
            " (%4d,%4d)"
        ...
        bRet = pParams->pEnumFunc( (HWND)pWnd, pParams->lParam );
        return bRet;
